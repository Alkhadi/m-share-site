 ...
 * 3) Adds a professional readability stylesheet (assets/css/readability-professional.css) that:
 *    - Forces readable contrast across the site
 *    - Default text is WHITE on dark/colored surfaces and BLACK on truly light/white surfaces
 *    - H1–H6, paragraphs and lists inherit current color (so titles and body text follow the surface)
 ...
 * - Titles/body text inherit current surface color (h1–h6, p, li)
...
/* Default: white text on dark */
html, body { color: var(--ink, #fff); }

/* Titles/body/list inherit current surface color */
h1,h2,h3,h4,h5,h6,
p, li, dt, dd, small, strong, em { color: inherit !important; }

/* Common LIGHT/WHITE surfaces switch to black ink */
.card:not(.dark),
.sheet-card:not(.dark),
.paper, .surface, .panel,
...
[style*="background:#fff"], [style*="background: #fff"], [style*="rgb(255, 255, 255)"],
[data-surface="light"] {
...
.badge, .chip { color:#fff !important; background: color-mix(in srgb, var(--primary), #fff 8%); border:1px solid var(--border-dark); }
